...
@Component
public class DAAssignmentDelayedStrategy implements DistributionStrategy {
    ...
    public void register() {
        distributionService.register(DistributionType.DAASSIGNMENT_DELAY_PRIORTIZED, this);
    }
    ...
    public void distribute(Order order, List<DeliveryAgent> agentList) {
        ...
        for (DeliveryAgent deliveryAgent : agentList) {
            if (!deliveryAgent.isAssigned()) {
                ...
                double cummulative = (0.2) * Constants.ORDER_DELAY_TIME_PROP_CONSTANT * (orderDelay) +
                        (0.1) * Constants.FIRST_MILE_PROP_CONSTANT / firstMile +
                        (0.7) * Constants.DA_WAITING_TIME_PROP_CONSTANT * daWaitingTime;
        ...
        if (assigned != null) {
            ...
            throw new SwiggyException("DeliveryAgent couldn't be assigned through DAAssignmentDelayedStrategy Strategy");
        }
